...
public class FileTruststoreProvider implements TruststoreProvider {
    ...
    private void init() {
        ...
        InputStream inputStream = getClass().getClassLoader().getResourceAsStream("truststore.properties");
        if (inputStream != null) {
            try {
        ...
        String storepath = config.getProperty("file", null);
        String pass = config.getProperty("password", null);
        String policy = config.getProperty("hostname-verification-policy", null);
        Boolean disabled = Boolean.parseBoolean(config.getProperty("disabled", null));
